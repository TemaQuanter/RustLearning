    ...
    loop {
        ...
        if let Ok(img) =  serde_json::from_str::<Image>(&response) {
            ...
            if l_image != "".to_string() {
                fs::remove_file(&l_image)
                    .expect("Failed to remove the last stored image");
            } // end if
            ...
            let mut img_file: File = fs::File::create(&l_image)
                .expect("Failed to create a file for an image");
